...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<List<String>>> handleMethodArgumentNotValidException(MethodArgumentNotValidException exception) {
        ...
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ApiResponse.fail(400, "参数校验失败", errors));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleHttpMessageNotReadableException(HttpMessageNotReadableException exception) {
        ...
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ApiResponse.fail(400, "请求体不是有效的 JSON 或字段类型不正确"));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleRequestBindingException(Exception exception) {

        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ApiResponse.fail(400, "请求参数格式错误或缺少必要参数"));
    }
