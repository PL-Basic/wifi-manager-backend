...
public class TrafficQueryServiceImpl implements TrafficQueryService {
    ...
    @Override
    public TrafficPageResult pageTraffic(Long tenantId, long current, long size, String mac, Long sessionId, String dstIp, LocalDateTime startTime, LocalDateTime endTime) {

        return page(tenantId, null, current, size, mac, sessionId, dstIp, startTime, endTime);
    }

    @Override
    public TrafficPageResult pageOwnedTraffic(Long tenantId, Long ownerUserId, long current, long size, String mac, Long sessionId, String dstIp, LocalDateTime startTime, LocalDateTime endTime) {

        if (ownerUserId == null || ownerUserId <= 0) {
            throw new IllegalArgumentException("缺少有效用户身份");
        }

        return page(tenantId, ownerUserId, current, size, mac, sessionId, dstIp, startTime, endTime);
    }

    private TrafficPageResult page(Long tenantId, Long ownerUserId, long current, long size, String mac, Long sessionId, String dstIp, LocalDateTime startTime, LocalDateTime endTime) {
